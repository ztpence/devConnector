...
module.exports = (passport) => { // passing in passport as param from server.js line 29
    passport.use(
        new JwtStrategy(opts, (jwt_payload, done) => { // using new jwt strategy and pass in options and what is given back is a function that gives us our payload and done
            User.findById(jwt_payload.id) // mongoose method and jwt payload is and object with id in it
            .then(user => { 
                if(user) {
